...
const AIDestinationFinder: React.FC = () => {
  ...
  return (
    <section id="ai-finder" className="py-20 bg-gray-900/50">
      <div className="container mx-auto px-6">
        <div className="bg-white/10 backdrop-blur-sm rounded-lg shadow-xl p-8 md:p-12 lg:flex lg:items-center lg:space-x-12 border border-[#F6520C]/20">
          <div className="lg:w-1/2">
            <h2 className="text-3xl md:text-4xl font-bold text-white">AI Destination Finder</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <select id="field" value={field} onChange={(e) => setField(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-3 text-base bg-gray-800/50 border-gray-600 focus:outline-none focus:ring-2 focus:ring-[#F6520C] focus:border-[#F6520C] sm:text-sm rounded-md text-white">
              ...
              <div>
                ...
                <select id="goal" value={goal} onChange={(e) => setGoal(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-3 text-base bg-gray-800/50 border-gray-600 focus:outline-none focus:ring-2 focus:ring-[#F6520C] focus:border-[#F6520C] sm:text-sm rounded-md text-white">
                  <option value="" disabled>Select a goal</option>
                  {careerGoals.map(g => <option key={g} value={g}>{g}</option>)}
                </select>
              </div>
              <button type="submit" disabled={loading} className="w-full bg-[#F6520C] text-white px-6 py-3 rounded-md font-semibold hover:bg-opacity-90 transition duration-300 disabled:from-gray-500 disabled:to-gray-600 disabled:cursor-not-allowed transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-[#E84A00]">
                {loading ? 'Analyzing...' : 'Find My Destination'}
              </button>
            </form>
          </div>
          <div className="lg:w-1/2 mt-8 lg:mt-0">
            <div className="bg-[#0a101f] rounded-lg p-6 min-h-[300px] border border-[#F6520C]/30 shadow-inner">
              ...
              {recommendation && <div className="text-gray-300 whitespace-pre-wrap font-mono">{recommendation}<span className="inline-block w-2 h-4 bg-[#F6520C] animate-pulse ml-1"></span></div>}
            </div>
          </div>
        </div>
      </div>
