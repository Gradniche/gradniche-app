...
const Header: React.FC<HeaderProps> = ({ navigate }) => {
  ...
  return (
    <header className={`bg-[#0a101f]/80 backdrop-blur-xl sticky z-50 md:mx-6 md:rounded-3xl transition-all duration-300 ${isScrolled ? 'top-0 md:top-4 shadow-[0_8px_30px_rgb(0,0,0,0.5)] border-b md:border border-white/10' : 'top-0 md:top-6 shadow-lg border-b md:border border-white/5'}`}>
              ...
              {link.isDropdown ? (
                <>
                  <button 
                    ...
                    className={`text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-300 font-medium flex items-center focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F6520C] rounded-full px-5 py-2.5 text-sm tracking-wide ${openDropdown === link.name ? 'bg-white/10 text-white shadow-inner' : ''}`}
                  >
                    {link.label}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[100] bg-[#0a101f] flex flex-col w-screen h-screen overflow-hidden md:hidden">
            ...
            <div className="h-[80px] flex-none w-full bg-[#0a101f]/95 backdrop-blur-md border-b border-white/5"></div>
