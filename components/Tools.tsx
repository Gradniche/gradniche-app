...
const toolsList = [
    { id: 'sop-analyzer', name: 'SOP Generator', icon: '📝', path: '/tools/sop-analyzer', color: 'from-orange-500 to-red-500' },
    ...
    { id: 'cost-calculator', name: 'Cost Calculator', icon: '🧮', path: '/tools/cost-calculator', color: 'from-yellow-500 to-orange-500' },
...
const Tools: React.FC<ToolsProps> = ({ navigate }) => {
    ...
    return (
        <section id="all-tools" className="py-32 relative bg-[#050810] overflow-hidden">
            {/* Cinematic Background */}
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-[0.02]"></div>
            <motion.div 
                animate={{ scale: [1, 1.1, 1], opacity: [0.02, 0.05, 0.02] }}
                transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] bg-gradient-to-tr from-blue-600/10 to-purple-600/10 rounded-full blur-[150px] pointer-events-none"
            ></motion.div>
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                >
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-md mb-8">
                        <span className="text-xs font-semibold tracking-widest text-gray-300 uppercase">Complete Arsenal</span>
                    </div>
                    <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 tracking-tighter" style={{ fontFamily: "'Playfair Display', serif" }}>
                ...
                >
                    {toolsList.map((tool) => (
                        <motion.button
                            ...
                            className="group relative flex flex-col items-center justify-center p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] hover:border-white/20 transition-all duration-300 overflow-hidden shadow-lg"
                        >
                            ...
                            <span className="text-white font-medium text-sm tracking-wide">{tool.name}</span>
                        </motion.button>
                    ))}
                </motion.div>
            </div>
